...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        func = {"rnn": (rnn_forward, rnn_backward), "lstm": (lstm_forward, lstm_backward)}
        fwd, bwd = func[self.cell_type]

        h0, cache_h0 = affine_forward(features, W_proj, b_proj)
        x, cache_x = word_embedding_forward(captions_in, W_embed)
        h, cache_h = fwd(x, h0, Wx, Wh, b)
        out, cache_out = temporal_affine_forward(h, W_vocab, b_vocab)

        loss, dL = temporal_softmax_loss(out, captions_out, mask)

        dL, dW_vocab, db_vocab = temporal_affine_backward(dL, cache_out)
        dL, dh0, dWx, dWh, db = bwd(dL, cache_h)
        dW_embed = word_embedding_backward(dL, cache_x)
    ...
    def sample(self, features, max_length=30):
        ...
        fwd = {
          "rnn": lambda x, h, c: (rnn_step_forward(x, h, Wx, Wh, b)[0], c),
          "lstm": lambda x, h, c: lstm_step_forward(x, h, c, Wx, Wh, b)[:2]
        }
