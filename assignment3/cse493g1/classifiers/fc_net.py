...
class FullyConnectedNetImproved(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
          W, b = self.params.get(f'W{i+1}', None), self.params.get(f'b{i+1}', None)
          gamma, beta = self.params.get(f'gamma{i+1}', None), self.params.get(f'beta{i+1}', None)
          bn_param = self.bn_params[i] if gamma is not None else None 
          dropout_param = self.dropout_param if self.use_dropout else None

          layer_input, caches[i+1] = regularization_forward(
            layer_input, W, b, gamma, beta, bn_param, dropout_param, i==self.num_layers-1
          )
        scores = layer_input
        ...
        for i in reversed(range(self.num_layers)):
          dL, dW, db, dgamma, dbeta = regularization_backward(dL, caches[i+1])
