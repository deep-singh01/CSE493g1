...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        caption_embeds = self.positional_encoding(self.embedding(captions))
        visual_features = self.visual_projection(features).unsqueeze(1)
        tgt_mask = torch.tril(torch.ones(T, T))
        trans_output = self.transformer(caption_embeds, visual_features, tgt_mask=tgt_mask)
        scores = self.output(trans_output)
